...
import { uploadMenuItemImage } from "@/lib/supabase/upload";
...
export function MenuItemImageUpload({
  ...
  async function handleFileSelect(files: FileList | null) {
    ...
    for (let i = 0; i < files.length && next.length < MAX_FILES; i++) {
      ...
      const result = await uploadMenuItemImage(orgId, formData);
    ...
    <div className={cn("space-y-2", className)}>
      <div className="flex flex-wrap items-start gap-3">
        ...
        <button
          ...
          disabled={disabled || uploading || urls.length >= MAX_FILES}
          className={cn(
            "flex size-24 shrink-0 flex-col items-center justify-center gap-1 rounded-lg border-2 border-dashed transition-colors",
        ...
        >
          ...
          <span className="text-[9px] text-muted-foreground/80">or drop</span>
        </button>
        {urls.map((url, index) => (
          <div
            key={url}
            ...
            className={cn(
              "relative size-24 shrink-0 cursor-grab overflow-hidden rounded-lg border bg-muted active:cursor-grabbing",
      ...
      <p className="text-muted-foreground text-xs">
        First image is the banner. Drag to reorder. Up to {MAX_FILES} images. JPEG, PNG, WebP. 5MB each.
      </p>
    </div>
  );
}
