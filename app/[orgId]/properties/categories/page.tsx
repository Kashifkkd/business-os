...
const columnHelper = createColumnHelper<PropertyCategory>();

export default function PropertyCategoriesPage() {
  const params = useParams();
  const orgId = params?.orgId as string;
  const { data: categories = [], isLoading } =
    usePropertyCategoriesList(orgId);
  const { data: allSubcategories = [] } = usePropertySubcategoriesList(orgId, {
  ...
  const createCat = useCreatePropertyCategory(orgId);
  const updateCat = useUpdatePropertyCategory(orgId);
  const deleteCat = useDeletePropertyCategory(orgId);
  ...
  const startEdit = (c: PropertyCategory) => {
    setEditingId(c.id);
    setEditName(c.name);
  };
    ...
    <div className="container mx-auto max-w-6xl p-4">
      <div className="mb-2">
        ...
        <p className="text-muted-foreground text-xs">
          Manage property categories (e.g. Residential, Commercial).
          Sub-categories are managed separately.
        </p>
      </div>
      ...
      <div className="flex flex-col gap-4">
        <div className="flex flex-wrap items-center justify-between gap-2">
          <p className="text-muted-foreground text-sm">
            {total === 0
              ? "No categories"
              : `Showing ${total} categor${total === 1 ? "y" : "ies"}`}
          </p>
        ...
        {isLoading ? (
          ...
          <EmptyState
            title="No categories"
            description="Add a category to organize your properties (e.g. Residential, Commercial)."
            icon={Layers}
            action={
              <Button
                size="sm"
      ...
      <Dialog open={createOpen} onOpenChange={setCreateOpen}>
        <DialogContent className="sm:max-w-md" showCloseButton>
          <DialogHeader>
            ...
            <DialogDescription>
              Create a new property category (e.g. Residential, Commercial).
            </DialogDescription>
          </DialogHeader>
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This will only succeed if the category has no sub-categories.
              Otherwise delete or move sub-categories first.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
