...
export async function GET(
  ...
  if (error || !row) {
    return NextResponse.json(apiError(API_ERROR_CODES.NOT_FOUND, "Company not found"), { status: 404 });
  }

  return NextResponse.json(apiSuccess(row as Company));
}

export type UpdateCompanyBody = { name?: string };
...
export async function PATCH(
  ...
  if (name !== undefined && !name) {
    return NextResponse.json(apiError(API_ERROR_CODES.VALIDATION_ERROR, "Company name cannot be empty"), { status: 400 });
  }
  ...
  if (Object.keys(payload).length === 0) {
    ...
    return NextResponse.json(apiSuccess(existing as Company));
  }
