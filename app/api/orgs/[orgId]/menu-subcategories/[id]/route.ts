...
export async function PATCH(
  ...
  if (Object.keys(payload).length === 0) {
    ...
    return NextResponse.json(existing as MenuSubCategory);
  }
  ...
  return NextResponse.json(row as MenuSubCategory);
}
...
export async function DELETE(
  ...
  if (count != null && count > 0) {
    return NextResponse.json(
      { error: "Cannot delete sub-category that is used by menu items. Change or remove items first." },
      { status: 400 }
    );
  }
